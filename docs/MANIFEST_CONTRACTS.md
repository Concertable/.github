...
(`commit`, `runUrl`), non-empty `images` entries (`name`, immutable `digest`), and `packages` entries
...
dependency object on one logical JSON object and do not replace digests with tags.
